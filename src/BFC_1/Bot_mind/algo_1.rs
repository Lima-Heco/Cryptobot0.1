pub mod BFC_1_mind {
	...
	pub fn findV1(view: &BFC_1_view, btc_price: Arc<RwLock<get_bitcoin::btcprice>>, wait: Arc<Mutex<bool>>){
		...
		if view.tableau[1].valeure_de_pente > 0.0 && view.tableau[0].valeure_de_pente < -20.0 && view.tableau[0].size > 3 && view.temp.valeure_de_pente > 20.0 && view.temp.size == 2 && *wait_cl.lock().unwrap() == false{
